...
- (BOOL)application:(UIApplication*)app openURL:(NSURL*)url options:(NSDictionary<NSString*, id>*)options
{
    ...
    if ([[url absoluteString] containsString:[self getWXAppId]]) {
        return [WXApi handleOpenURL:url delegate:[WechatForUnity wechatInstance]];
    }else{
        if([TencentOAuth CanHandleOpenURL:url]) {
            [QQApiInterface handleOpenURL:url delegate:(id<QQApiInterfaceDelegate>)[QQForUnity class]];
...
- (NSString*)getWXAppId{
    if (mWXAppId == nil) {
        ...
        NSArray *array = [infoDict objectForKey:@"CFBundleURLTypes"];
        
        for (int i = 0; i < array.count; i++)
        {
            if([[array[i] objectForKey:@"CFBundleURLName"] isEqualToString:@"weixin"]) {
                mWXAppId = [array[i] objectForKey:@"CFBundleURLSchemes"][0];
                break;
            }
        }
    }
    ...
    if([userActivity.activityType isEqualToString:NSUserActivityTypeBrowsingWeb]) {
        ...
        if(url){
            if([TencentOAuth CanHandleUniversalLink:url]) {
                [QQApiInterface handleOpenUniversallink:url delegate:(id<QQApiInterfaceDelegate>)[QQForUnity class]];
                return [TencentOAuth HandleUniversalLink:url];
            }
            if ([[url absoluteString] containsString:[self getWXAppId]]) {
                return [WXApi handleOpenUniversalLink:userActivity delegate:[WechatForUnity wechatInstance]];
            }
        }
    }
    return YES;
