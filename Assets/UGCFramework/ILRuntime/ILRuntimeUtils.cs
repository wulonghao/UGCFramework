...
public class ILRuntimeUtils
{
    ...
    public static void LoadHotFixWithInit()
    {
        ...
        if (hotfixAB)
        {
            TextAsset taHotFix = hotfixAB.LoadAsset<TextAsset>("dxzkhotfix");
            if (!taHotFix) return;
            using (MemoryStream fs = new MemoryStream(taHotFix.bytes))
            {
                if (MiscUtils.GetChannel() == "Test")
                {
                    TextAsset taHotFixPdb = hotfixAB.LoadAsset<TextAsset>("dxzkhotfixpdb");
                    if (!taHotFixPdb) return;
                    using (MemoryStream p = new MemoryStream(taHotFixPdb.bytes))
                    {
