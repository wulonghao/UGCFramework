...
public class SocketClient : MonoBehaviour
{
    ...
    public void AddSendMessageQueue(Msg_C2S c2s, bool isPrevent = false)
    {
        if (isPrevent)
            LoadingPnl.OpenLoading();
        lock (tempSendList)
        {
    ...
    bool Send(Msg_C2S c2s)
    {
        ...
        try
        {
            int sendLength = mSocket.Send(BuildPackage(c2s), SocketFlags.None);
    ...
    /// 构建消息数据包
    /// </summary>
    byte[] BuildPackage(Msg_C2S c2s)
    {
        byte[] b = ProtobufSerilizer.Serialize(c2s);
        if (b == null)
            return new byte[0];
        //TODO 包体结构自定义
        ProtobufByteBuffer buf = ProtobufByteBuffer.Allocate(b.Length);
        //buf.WriteBytes(b);
        return buf.GetBytes();
    }
    #endregion
    ...
    {
        TipManager.Instance.OpenTip(TipType.AlertTip, "与服务器断开连接", 0, () => { /*退出登录*/ });
    }
